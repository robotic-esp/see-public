...
void SeeCore::InitialisePoints(int size) {
  ...
  vc_.distance.resize(size, FLT_MAX);
  vc_.switched.resize(size, true);
  vc_.scale.resize(size, 1);
...
void SeeCore::AddCorePoint(int pt_idx) {
  if (see_pt_ptr_->points[pt_idx].label != core_pt) {
    ...
    see_pt_ptr_->points[pt_idx].r = 0;
    see_pt_ptr_->points[pt_idx].g = 0;
    see_pt_ptr_->points[pt_idx].b = 255;
...
void SeeCore::AddFrontierPoint(int pt_idx) {
  if (see_pt_ptr_->points[pt_idx].label != frontier_pt) {
    ...
    see_pt_ptr_->points[pt_idx].r = 255;
    see_pt_ptr_->points[pt_idx].g = 0;
    see_pt_ptr_->points[pt_idx].b = 0;
...
void SeeCore::AddOutlierPoint(int pt_idx) {
  if (see_pt_ptr_->points[pt_idx].label != outlier_pt) {
    ...
    see_pt_ptr_->points[pt_idx].r = 0;
    see_pt_ptr_->points[pt_idx].g = 255;
    see_pt_ptr_->points[pt_idx].b = 0;
...
void SeeCore::ViewController(SeePointCloudPtr see_pt_dash_ptr) {
  ...
  u_x_f << 0, -f_b(2), f_b(1), f_b(2), 0, -f_b(0), -f_b(1), f_b(0), 0;
  u_x_b << 0, -f_f(2), f_f(1), f_f(2), 0, -f_f(0), -f_f(1), f_f(0), 0;
  ...
  } else if (f_d.norm() < vc_.distance[f_idx_]) {
    theta_f = vc_.scale[f_idx_] * f_d(2) * see_.d;
    theta_f /= pow(see_.d, 2) + (vc_.scale[f_idx_] + 1) * pow(f_d(2), 2);
    theta_f = atan(theta_f);
    R_f = Eigen::Matrix3f::Identity() + sin(theta_f) * u_x_f;
    R_f += (1 - cos(theta_f)) * (u_x_f * u_x_f);

    theta_b = vc_.scale[f_idx_] * f_d(1) * see_.d;
    theta_b /= pow(see_.d, 2) + (vc_.scale[f_idx_] + 1) * pow(f_d(1), 2);
    theta_b = atan(theta_b);
    R_b = Eigen::Matrix3f::Identity() + sin(theta_b) * u_x_b;
    R_b += (1 - cos(theta_b)) * (u_x_b * u_x_b);

    v_p = R_b * (v_p + (vc_.scale[f_idx_] + 1) * f_d(1) * f_f);
    v_p = R_f * (v_p + (vc_.scale[f_idx_] + 1) * f_d(2) * f_b);

    vc_.scale[f_idx_] *= 2;
    vc_.distance[f_idx_] = f_d.norm();

    ROS_INFO_STREAM("Adjusting view");
    AdjustView(f_idx_, v_p);
  } else if (vc_.switched[f_idx_]) {
...
void SeeCore::SwitchView(int pt_idx) {
  vc_.switched[pt_idx] = false;
  vc_.distance[pt_idx] = FLT_MAX;
  vc_.scale[pt_idx] = 1;
